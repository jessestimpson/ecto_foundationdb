defmodule Ecto.Adapters.FoundationDB do
  ...
  ** Between queries are supported for these types:**
  ...
  Versionstamp primary keys are strictly ordered across the entire table. When you query a range of records
  for a particular user or entity (e.g. all sessions for `"alice"`), FoundationDB must scan from a starting
  key through all records until the desired end — potentially reading records belonging to many other users
  along the way.
  ...
  Repo.all(
    from(s in Session, where: s.id == ^{"alice", session_id}),
    prefix: tenant
  )
  ```
