defmodule EctoFoundationDB.Layer.Query do
  ...
  defp make_datakey_range(
    ...
    # When only the left bound specifies a partition, the right bound defaults to
    # the end of the same partition (not the entire table).
    eff_right_partition =
      if right_partition == @no_partition, do: left_partition, else: right_partition

    {left_range_start, left_range_end} =
      if is_nil(actual_left) do
        build_primary_range(tenant, plan.source, left_partition)
      else
        PrimaryKVCodec.range(build_primary_codec(tenant, plan.source, left_partition, actual_left))
      end

    {right_range_start, right_range_end} =
      if is_nil(actual_right) do
        build_primary_range(tenant, plan.source, eff_right_partition)
      else
        PrimaryKVCodec.range(
          build_primary_codec(tenant, plan.source, eff_right_partition, actual_right)
        )
      end
